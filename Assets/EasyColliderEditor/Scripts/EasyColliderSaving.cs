...
namespace ECE
{
  public static class EasyColliderSaving
  {
    ...
    public static string GetValidConvexHullPath(GameObject go)
    {
      ...
      if (ECEPreferences.ConvexHullSaveMethod != CONVEX_HULL_SAVE_METHOD.Folder)
      {
        ...
        if (altPath != null && altPath != "" && foundObject != null)
        {
          ...
          if (index >= 0)
          {
            ...
            // add back in the object name.
            path += foundObjectName;
          }
        }
      }
      ...
      // if the saving directory or asset database path doesn't exist (prefab/mesh saving mehtods), looks like we fall back to the ECEPrefences.SaveConvexHullPath location.
